import { DeviceUtils } from 'src/core/utils';
...
const isMac = DeviceUtils.isBrowser && /Mac/.test( navigator.platform );
const isPointerEventsNotSupported = ! ( DeviceUtils.isBrowser && 'PointerEvent' in window ); // macOS Safari 12 does not support PointerEvents API
...
export class CameraControls extends EventDispatcher {
	...
	protected _elementRect = DeviceUtils.isBrowser ? new DOMRect() : {
        x: 0,
        y: 0,
        width: 0,
        height: 0,
	...
	constructor(
		...
		const onPointerDown = ( event: EventInfo ) => {
            ...
            // @ts-ignore
            this._domElement.addEventListener("pointerup", onPointerUp);
		...
		const onMouseDown = ( event: EventInfo ) => {
		...
		const onPointerMove = ( event: EventInfo ) => {
		...
		const onMouseMove = ( event: EventInfo ) => {
		...
		const onPointerUp = ( event: EventInfo ) => {
		...
		const onMouseWheel = ( event: EventInfo ): void => {
		...
		const onContextMenu = ( event: EventInfo ): void => {
		...
		const startDragging = ( event: EventInfo ): void => {
		...
		const dragging = (): void => {
			...
			const isPointerLockActive = DeviceUtils.isBrowser ? this._domElement && document.pointerLockElement === this._domElement : false;
	...
	protected _truckInternal = ( deltaX: number, deltaY: number, dragToOffset: boolean ): void => {
		...
		if ( isPerspectiveCamera( this._camera ) ) {
			...
			// orthographic
			const camera = this._camera;
            // TODO:maybe cameracontrols has bug with truckSpeed
			const truckX    = this.truckSpeed * deltaX * ( camera.right - camera.left   ) / camera.zoom / this._elementRect.width;
			const pedestalY = this.truckSpeed * deltaY * ( camera.top   - camera.bottom ) / camera.zoom / this._elementRect.height;
			dragToOffset ?
	...
	static createBoundingSphere( object3d: _THREE.Object3D, out: _THREE.Sphere = new THREE.Sphere() ): _THREE.Sphere {
		...
		object3d.traverseVisible( ( object ) => {
			...
			if ( geometry.isBufferGeometry ) {
				...
				const position = geometry.attributes.position as _THREE.BufferAttribute | _THREE.InterleavedBufferAttribute;
